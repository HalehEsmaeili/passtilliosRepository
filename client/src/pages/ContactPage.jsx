...
import thankYouVid from "../public/images/contactPage/thankyou.mp4";
    ...
    setFormData({
     ...
     console.log("cccccccccc"+email);
      // Make a POST request to your Node.js server
       const response = await axios.post('http://localhost:3001/api/contact/saveEmail', {
        email: email, 
        name:name
       ...
       if (response.status=== 200) {
        ...
        if (response.data.errors && response.data.errors.length > 0) {
          ...
          if (firstError.error_code === 'ERROR_GENERIC' && firstError.field === 'ADDRESS') {
...
it is about time to embrace the true charm of grey!
lets step by step guide it back to being the background color; the perfect backdrop where vibrant and unique colors can take the lead as the main storytellers in the painting of our lives!
</p>
 </div>
...
<main className="form-signin w-100 m-auto">
 ...
 {/**<form action="/" method="post"> */ }
    <div className="mb-3">
      <h1 className="h3 mb-3 fw-normal h1contact">join the contact list!</h1>
    </div>
{(savedSuccessfully?
(<div className="successContainer">
<video id="chicks"  autoplay="autoplay" muted loop> <source src={thankYouVid} type="video/mp4"></source>  </video> 
</div>)
...
<div className="mb-3">
      ...
      <input type="text" name="name" className="form-control" id="name" onChange={handleChange} value={name} placeholder="my name..." />
     {(btnPressed&& name==="")?
      <p className="errorP">forgot to enter your name?</p>:null
     }
    ...
    <div className="mb-3">
      <label htmlFor="e_mail" className="form-label">your Email:</label>
     
      <input type="email" name="email" className="form-control" id="e_mail" onChange={handleChange} value={email} placeholder="name@example.com" />
      
      {(btnPressed&& email==="")?
      <p className="errorP">forgot to enter your email?</p>:null
     }
    </div>


<Button id="contactBtn" name="count me in!" handleClick={handleClick}/>
